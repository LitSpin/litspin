...
#define FPGA_DMAC_QSYS_ADDRESS 0x00400
#define FPGA_DMAC_ADDRESS ((uint8_t*)HPS_FPGA_BRIDGE_BASE+FPGA_DMAC_QSYS_ADDRESS)
//Address of the On-Chip RAM in the FPGA, as seen by processor
#define FPGA_LED_QSYS_ADDRESS_UP 0x0
#define FPGA_LED_ADDRESS_UP ((uint8_t*)HPS_FPGA_BRIDGE_BASE+FPGA_LED_QSYS_ADDRESS_UP)
//Address of the leds in the FPGA, as seen by DMAC
#define FPGA_LED_ADDRESS_DMAC 0x0
...
//#define WRITE_OPERATION
  ...
  #define DMA_TRANSFER_DST_DMAC   ((uint8_t*) FPGA_LED_ADDRESS_DMAC)
  #define DMA_TRANSFER_DST_UP     ((uint8_t*) FPGA_LED_vaddr)
#endif
...
int main() {
  ...
  void *FPGA_LED_vaddr_void = virtual_base
  + ((unsigned long)(FPGA_LED_QSYS_ADDRESS_UP) & (unsigned long)( HW_REGS_MASK ));
  uint8_t* FPGA_LED_vaddr = (uint8_t *) FPGA_LED_vaddr_void;
 ...
 //Reset HPS-OCR
  ...
  printf("\nWRITE: Copy %d Bytes from HPS OCR (%x) to leds (%x)\n",
    (int) DMA_TRANSFER_SIZE, (unsigned int) DMA_TRANSFER_SRC_UP,
    (unsigned int) DMA_TRANSFER_DST_UP);

  for (i=0; i<DMA_TRANSFER_SIZE;i++) DMA_TRANSFER_SRC_UP[i] = (uint8_t)(1);
  printf("HPS before DMA transfer = ");
  printbuff(DMA_TRANSFER_SRC_UP, DMA_TRANSFER_SIZE);
  printf("Leds OCR before DMA transfer = ");
  ...
  fpga_dma_write_reg( FPGA_DMA_vaddr_void,  //set destination address
  ...
  printf("HPS after DMA transfer = ");
  printbuff(DMA_TRANSFER_SRC_UP, DMA_TRANSFER_SIZE);
  printf("Leds after DMA transfer = ");
  printbuff(DMA_TRANSFER_DST_UP, DMA_TRANSFER_SIZE);
